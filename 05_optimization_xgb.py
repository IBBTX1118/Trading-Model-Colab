...
class MLOptimizerAndBacktester:
    ...
    def run_for_single_market(self, market_file_path: Path):
        ...
        params_filename = self.output_base_dir / f"{market_name}_best_params_xgb.json" # 修改檔名以區分
    ...
    def run(self):
        self.logger.info(f"{'='*25} XGBoost 整合式滾動優化與回測流程開始 (版本 13.0) {'='*25}")
