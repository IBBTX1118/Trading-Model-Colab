...
class FinalMLStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        prob_sl, prob_tp = pred_probs[0], pred_probs[1]; current_price, atr_value = self.data.close[0], self.atr[0]
...
class MLOptimizerAndBacktester:
    ...
    def objective(self, trial: optuna.trial.Trial, X_train, y_train, df_val, available_features: list, market_name: str) -> float:
        ...
        if "USDJPY" in market_name:
        ...
        elif "GBPUSD" in market_name:
            self.logger.debug(f"為 {market_name} 應用精細化參數範圍。")
            model_param['n_estimators'] = trial.suggest_int('n_estimators', 100, 600)
            ...
            model_param['n_estimators'] = trial.suggest_int('n_estimators', 100, 800)
        ...
        model_param['lambda_l1'] = trial.suggest_float('lambda_l1', 1e-8, 10.0, log=True)
        model_param['lambda_l2'] = trial.suggest_float('lambda_l2', 1e-8, 10.0, log=True)
    ...
    def run_for_single_market(self, market_file_path: Path):
        ...
        while current_date + wfo_days['training_days'] + wfo_days['validation_days'] + wfo_days['testing_days'] <= end_date:
            ...
            model_params = {k: v for k, v in study.best_params.items() if k not in self.strategy_params.keys()}; model_params.update({'objective': 'multiclass', 'metric': 'multi_logloss', 'num_class': 3, 'verbosity': -1})
            final_model = lgb.LGBMClassifier(**model_params); final_model.fit(X_train, y_train)
            
            final_test_params = self.strategy_params.copy()
            for k in ['entry_threshold', 'tp_atr_multiplier', 'sl_atr_multiplier']: final_test_params[k] = study.best_params.get(k, self.strategy_params[k])
        ...
        params_filename = self.output_base_dir / f"{market_name}_best_params.json"
...
if __name__ == "__main__":
    try:
        config = load_config(); optimizer = MLOptimizerAndBacktester(config); optimizer.run()
