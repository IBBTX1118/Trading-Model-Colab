# 檔名: 05_model_debugger.ipynb (v2.0 - 信心視覺化版)
import pandas as pd
...
confident_buys = np.sum(win_probabilities > 0.6) # 大於 60% 的信號
confident_sells = np.sum(win_probabilities < 0.4) # 小於 40% 的信號 (認為會輸)
...
print("\n如何解讀這張圖:")
print("  - 【理想情況】: 圖形應呈 'U' 型或雙峰型，大量信號集中在 <20% 和 >80% 的區域，表示模型對大部分情況都很有信心。")
print("  - 【當前可能的問題】: 圖形可能是一個又高又窄的尖峰，集中在 50% 附近。這表示模型對所有情況都很困惑，無法區分好壞，信心約等於隨機猜測。")
