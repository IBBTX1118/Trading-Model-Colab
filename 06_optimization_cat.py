# 檔名: 06_optimization_cat.py
# 描述: 使用 CatBoost 模型進行參數優化與回測。
# 版本: 13.0 (CatBoost 實驗版)
...
import catboost as cb # ★★★ 導入 catboost ★★★
import optuna
...
class MLOptimizerAndBacktester:
    ...
    def objective(self, trial: optuna.trial.Trial, X_train, y_train, df_val, available_features: list, market_name: str) -> float:
        ...
        model_param = {
            ...
            'l2_leaf_reg': trial.suggest_float('l2_leaf_reg', 1e-8, 10.0, log=True), # L2 正則化
            'random_strength': trial.suggest_float('random_strength', 1e-8, 10.0, log=True), # 避免過擬合
        }
        ...
        # ★★★ 核心修改：改用 CatBoostClassifier ★★★
        model = cb.CatBoostClassifier(**model_param)
    ...
    def run_for_single_market(self, market_file_path: Path):
        ...
        while current_date + wfo_days['training_days'] + wfo_days['validation_days'] + wfo_days['testing_days'] <= end_date:
            ...
            model_params.update({'loss_function': 'MultiClass', 'verbose': 0, 'random_seed': 42})
            
            # ★★★ 核心修改：使用 CatBoostClassifier 訓練最終模型 ★★★
        ...
        params_filename = self.output_base_dir / f"{market_name}_best_params_cat.json" # 修改檔名以區分
    ...
    def run(self):
        self.logger.info(f"{'='*25} CatBoost 整合式滾動優化與回測流程開始 (版本 13.0) {'='*25}")
